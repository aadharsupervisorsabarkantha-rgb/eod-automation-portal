...
.success-card{ padding:20px; border-radius:12px; background:#f0fdf4; border:1px solid #16a34a; color:#166534; margin-top:15px; font-size:16px; line-height:1.8; }
        ...
        for uploaded_file in uploaded_files:
            with pyzipper.AESZipFile(uploaded_file) as zf:
                ...
                for html_file in html_files:
                    ...
                    station_id, operator_id, all_entries = None, None, []
                    ...
                    for row in rows:
                        ...
                        # Data row filtering (Minimum 10 columns required)
                        if len(tds) >= 15:
                            eid_text = tds[1] # Enrolment column
                            digits = re.sub(r"\D", "", eid_text)
                            
                            if len(digits) >= 20:
                                try:
                                    h_date = datetime.strptime(digits[-14:-6], "%Y%m%d").strftime("%d/%m/%Y")
                                    txn_type = tds[3].strip().upper() if len(tds) > 3 else "E"
                                    
                                    # --- FIX: FINAL AMOUNT CALCULATION ---
                                    # Hum humesha last column (Total Amount) uthayenge
                                    amt_str = tds[-1].replace("Rs.", "").replace("Rs", "").replace(",", "").strip()
                                    try:
                                        f_amt = float(amt_str)
                                        # Security: Agar amount 200 se zyada hai (jaise EID number), toh zero kar do
                                        if f_amt > 200: f_amt = 0.0 
                                    except:
                                        f_amt = 0.0
                    ...
                    if station_id and all_entries:
                        ...
                        # Existing Dates Sync (Normalizing Slash and Dash)
                        existing_data = worksheet.get_all_values()
                        existing_dates = [r[0].replace('-', '/') for r in existing_data if len(r) > 0]
                        
                        unique_dates = sorted(list(set(df["date"].tolist())), key=lambda x: datetime.strptime(x, "%d/%m/%Y"))
                        newly_added, duplicate_dates = [], []
                        ...
                        for d in unique_dates:
                            ...
                            total = len(day_df)
                            amount = int(day_df["amt"].sum()) # Correct Total Amount
                            
                            worksheet.append_row([d.replace('/', '-'), station_id, op_name, operator_id, enrol, update, total, amount])
                            newly_added.append(d)

                        # UI Display
                        if not newly_added and duplicate_dates:
                            st.warning(f"⚠️ Data already exists for {duplicate_dates[0]} to {duplicate_dates[-1]}")
                        elif newly_added:
                            st.markdown(f'<div class="success-card"><h3>✅ DATA SAVED</h3>👤 {op_name} | 📍 Station: {station_id}</div>', unsafe_allow_html=True)
                            st.success(f"Saved: {newly_added[0]} to {newly_added[-1]}")
                            
                            # Final Stats Table
                            res_df = df[df["date"].isin(newly_added)].groupby("date").agg(
                                Enrol=('type', lambda x: (x=='E').sum()),
                                Update=('type', lambda x: (x=='U').sum()),
                                Total=('type', 'count'),
                                Amount=('amt', 'sum')
                            ).reset_index()
                            st.table(res_df)
                        ...
                        st.error("⚠️ No transaction data found in the HTML file.")
